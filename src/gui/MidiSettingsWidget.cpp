...
AdditionalMidiSettingsWidget::AdditionalMidiSettingsWidget(QSettings *settings, QWidget *parent)
    ...
    QWidget *tpqInfo = createInfoBox("Note: There aren't many reasons to change this. MIDI files have a resolution for "
                                     "how many ticks can fit in a quarter note. Higher values = more detail. Lower "
                                     "values may be required for compatibility. Only affects new files.");
    ...
    QWidget *playerModeInfo =
        createInfoBox("Note: the above option should not be enabled in general. It is only required if the stop button "
                      "does not stop playback as expected (e.g. when some notes are not stopped correctly).");
    ...
    QWidget *startCmdInfo = createInfoBox(
        "The start command can be used to start additional software components (e.g. Midi synthesizers) each time, "
        "MidiEditor is started. You can see the output of the started software / script in the field below.");
...
MidiSettingsWidget::MidiSettingsWidget(QWidget *parent) : SettingsWidget("Midi I/O", parent) {
    ...
    QWidget *playerModeInfo = createInfoBox(
        "Choose the Midi ports on your machine to which MidiEditor connects in order to play and record Midi data.");
    layout->addWidget(playerModeInfo, 0, 0, 1, 6);

    // output
    layout->addWidget(new QLabel("Midi output: ", this), 1, 0, 1, 2);
    _outList = new QListWidget(this);
    connect(_outList, SIGNAL(itemChanged(QListWidgetItem *)), this, SLOT(outputChanged(QListWidgetItem *)));
    ...
    connect(reloadOutputList, SIGNAL(clicked()), this, SLOT(reloadOutputPorts()));
    reloadOutputPorts();

    // input
    layout->addWidget(new QLabel("Midi input: ", this), 1, 3, 1, 2);
    _inList = new QListWidget(this);
    connect(_inList, SIGNAL(itemChanged(QListWidgetItem *)), this, SLOT(inputChanged(QListWidgetItem *)));
...
void MidiSettingsWidget::reloadInputPorts() {
    ...
    disconnect(_inList, SIGNAL(itemChanged(QListWidgetItem *)), this, SLOT(inputChanged(QListWidgetItem *)));
    ...
    foreach (QString name, MidiInput::inputPorts()) {
        ...
        QListWidgetItem *item = new QListWidgetItem(name, _inList, QListWidgetItem::UserType);
...
void MidiSettingsWidget::reloadOutputPorts() {
    ...
    disconnect(_outList, SIGNAL(itemChanged(QListWidgetItem *)), this, SLOT(outputChanged(QListWidgetItem *)));
    ...
    foreach (QString name, MidiOutput::outputPorts()) {
        ...
        QListWidgetItem *item = new QListWidgetItem(name, _outList, QListWidgetItem::UserType);
        item->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled | Qt::ItemIsUserCheckable);
