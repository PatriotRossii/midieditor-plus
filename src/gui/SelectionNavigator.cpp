...
void SelectionNavigator::navigate(qreal searchAngle) {
    ...
    for (int channelNumber = 0; channelNumber < 19; channelNumber++) {
        ...
        foreach (MidiEvent *channelEvent, channel->eventMap()->values()) {
            if (channelEvent == selectedEvent)
                continue;
            if (channelEvent->track()->hidden())
                continue;
            if (!eventIsInVisibleTimeRange(channelEvent))
                continue;
            if (!eventsAreSameType(selectedEvent, channelEvent))
                continue;
            qreal channelEventDistance =
                getDisplayDistanceWeightedByDirection(selectedEvent, channelEvent, searchAngle);
            if (channelEventDistance < 0.0)
                continue;
...
bool SelectionNavigator::eventsAreSameType(MidiEvent *event1, MidiEvent *event2) {
    ...
    if (noteOnEvent1 || noteOnEvent2)
        return (noteOnEvent1 && noteOnEvent2);
    OffEvent *offEvent1 = dynamic_cast<OffEvent *>(event1);
    OffEvent *offEvent2 = dynamic_cast<OffEvent *>(event2);
    if (offEvent1 || offEvent2)
        return (offEvent1 && offEvent2);
 ...
 */
qreal SelectionNavigator::getDisplayDistanceWeightedByDirection(MidiEvent *originEvent, MidiEvent *targetEvent,
                                                                qreal searchAngle) {
    MatrixWidget *matrixWidget = mainWindow->matrixWidget();
