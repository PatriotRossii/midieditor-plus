...
AppearanceSettingsWidget::AppearanceSettingsWidget(QWidget *parent) : SettingsWidget("Appearance", parent) {
    ...
    for (int i = 0; i < 17; i++) {
        ...
        NamedColorWidgetItem *channelItem = new NamedColorWidgetItem(i, name, *Appearance::channelColor(i), this);
    ...
    for (int i = 0; i < 16; i++) {
        NamedColorWidgetItem *trackItem =
            new NamedColorWidgetItem(i, "Track " + QString::number(i), *Appearance::trackColor(i), this);
        QListWidgetItem *item = new QListWidgetItem();
        item->setSizeHint(QSize(0, ROW_HEIGHT));
